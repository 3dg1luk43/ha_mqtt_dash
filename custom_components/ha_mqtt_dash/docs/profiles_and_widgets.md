...
- `type`: string — one of: `light`, `switch`, `scene`, `sensor`, `value`, `person`, `button`, `label`, `clock`, `weather`, `climate`, `camera`, `printer` (alias: `printer3d`), `spacer`
...
- climate: For `climate.*` entities. Shows current/target temperature and a mode selector. Optional `modes` limits selectable HVAC modes (e.g., `["off","heat","cool","auto"]`). Integration supplies `attr_base` and `extra_topics` for `current_temperature` and `temperature`. Actions published: `set_hvac_mode` and `set_temperature`.
- camera: Non-entity widget rendering an MJPEG stream. Provide `stream_url`. Optional `overlay_button` allows a single action button overlaid on the stream: `{ "label": "Door", "action": "press", "entity_id": "script.open_door" }`. The integration derives `overlay.command_topic` when `entity_id` is provided.
- printer/printer3d: Composite widget bound to multiple entities. Provide entity ids in fields: `nozzle_entity`, `bed_entity`, `time_entity`, `progress_entity`, `status_entity`. Optional `progress_unit` (e.g., "%"). The app subscribes to all referenced entities and renders a single panel. Time is displayed as `HH:MM` from a numeric minutes value.
...
Climate widget (modes + target temperature):
```json
{
    "id": "cl1",
    "type": "climate",
...
Camera widget with overlay button:
```json
{
    "id": "cam1",
    "type": "camera",
